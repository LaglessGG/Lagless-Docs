...
description: Unable to Access Jarfile? this is how you fix it.
---

## Introduction

Work in Progress article. This will be updated over time.
...
| Folia | Folia is a  fork of Paper that adds multithreading to the server for high playercounts, but breaks most plugins | https://papermc.io/software/folia/ |
...
For modded go by the modpack you wish to use.
...
`use-alternate-keepalive: false` the default  is fine. However, If you have players being timed out this may help as it does not kick players unless they have not responded to the server for 30 seconds.
